...
3. Run hosted UI validation when menu structure, copy, or menu-state behavior changes.
...
- Hosted UI validation: deterministic menubar snapshot shape and hosted screenshots.
...
- `make verify-ui-live` must write at least `summary.json`, `runtime-assertions.json`, `live-menu-snapshot.json`, and a screenshot

## Agent execution

- Use `make verify-ui` for hosted deterministic menu validation.
...
- status item content fallback and disabled menu-state behavior:
  - unit plus hosted UI validation
- inactive account rendering and switch-action wiring metadata:
  - hosted UI plus `SCENARIO=live-menu-open make verify-ui-live`
...
- real inactive-account click dispatch when Accessibility cannot enumerate the open menubar menu on the current machine
...
- `owner_layer`: `unit`
- `proofs_required`: `["unit", "hosted_ui", "live_ui"]`
...
- `owner_layer`: `live_ui`
- `proofs_required`: `["hosted_ui", "live_ui"]`
