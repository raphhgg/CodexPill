...
`MenuBarValidationObserver` is observer instrumentation for menubar validation. It owns translation from runtime events to validation event names, invariant IDs, payload sanitization, menu snapshot/action-trace recording, and file sink forwarding. Production coordinators should call semantic observer methods only; they should not know validation artifact formats. The observer is disabled/no-op in normal production paths when validation configuration is absent. Together with `.kite/scenarios/<scenario-id>.json`, `.kite/scenarios.json`, `MenuBarValidationSupport`, `MenuBarValidationScenarioFixtures`, `InMemoryRemoteHostClient`, `NoopCodexAppProcessClient`, and `ValidationFixtureBootstrap`, this is CodexPill's product-owned validation adapter surface, not generic Kite Harness runtime code.
