...
Feature risk: `ui_visual`, `parser`, `state_truth`, `privacy`, `performance`
...
with supporting `privacy-leak-proof`
...
- Kite scenario receipt for `token-usage-parser-aggregation`.
- focused test output from `DiagnosticReportBuilderTests`;
- `build/verification/token-usage-privacy-no-raw-session/diagnostics-export.json`;
- `build/verification/token-usage-privacy-no-raw-session/privacy-leak-report.json`;
- Kite scenario receipt for `token-usage-privacy-no-raw-session`.
...
- Does not prove live Token Usage runtime lifecycle or duplicate-scan
  prevention.
