...
| --- | --- | --- | --- | --- | --- |
| Denied macOS permission disables notification modes and routes to System Settings without deleting preferences. | `unit` plus `deterministic-ui` | Notification permission state tests and future menu projection proof. | Test result plus optional hosted menu artifact. | Denied permission renders mode rows unchecked/disabled, shows `Enable in macOS Settings…`, opens System Settings when selected, and preserves saved CodexPill notification preferences. | Synthetic permission/preferences only; no private system identifiers, account data, auth payloads, tokens, paths, emails, or hostnames. |
| `Account Available` fires only for inactive fallback accounts becoming useful again. | `unit` | Notification policy tests with account availability fixtures. | Policy fixture result bundle. | First-saved, only-saved, already-active, and non-fallback accounts do not trigger; inactive fallback accounts becoming useful do trigger once. | Synthetic account/rate-limit fixtures only; no raw auth, account identifiers, emails, hostnames, paths, or tokens. |
