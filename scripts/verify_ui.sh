...
if [[ "${SCENARIO}" != "hosted-menu-default" ]]; then
  case "${SCENARIO}" in
    hosted-menu-busy|hosted-menu-empty|hosted-menu-with-host|hosted-menu-multiple-hosts|hosted-menu-disconnected-host|host-account-missing-on-host)
      ;;
    live-menu-open|live-account-switch|live-remote-host-switch|live-add-host-prompt|live-save-current-account-name-dialog-cancelled|live-save-current-prompt|live-add-account-name-dialog-cancelled|live-sign-in-another-prompt|live-scheduled-refresh)
      ARTIFACT_ROOT="${BUILD_ROOT}/verification/${AGENT_NAME}/${SCENARIO}" \
